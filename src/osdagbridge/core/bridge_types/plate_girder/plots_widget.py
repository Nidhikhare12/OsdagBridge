...
def build_figure_sfd_contour(ds, force_key, nodes, members, show_grid=True, scale=1.0, selected_girder="All"):
    ...
    for elems in girders.values():
        _, _, _, v, _ = build_polyline(elems, comp_i, comp_j)
        vfull.extend(v)
    ...
    for i, (gid, elems) in enumerate(sorted_girders):
        ...
        xs, ys, zs, v, node_ids = build_polyline(elems, comp_i, comp_j)

        if max(v) - min(v) == 0:
            shear_scale = 1.0 if max(v) == 0 else 0.25 * abs((max(xs) - min(xs)) / max(v))
        else:
            shear_scale = 0.25 * abs((max(xs) - min(xs)) / (max(v) - min(v)))
        
        shear_scale *= scale

        x_step = np.repeat(xs, 2)[1:-1]
        v_step = np.repeat(v[:-1], 2)
        y_plot = v_step * shear_scale
        z_step = [np.mean(zs)] * len(y_plot)

        fig.add_trace(go.Surface(
            x=[x_step, x_step], y=[np.zeros(len(y_plot)), y_plot], z=[z_step, z_step],
            surfacecolor=[v_step, v_step], colorscale="Viridis", cmin=min(vfull), cmax=max(vfull),
            opacity=0.4, showscale=False, hoverinfo="skip", name=girder_name, legendgroup=girder_name, showlegend=False
        ))

        fig.add_trace(go.Scatter3d(
            x=x_step, y=y_plot, z=z_step, mode="lines",
            line=dict(width=6, color=v_step, colorscale="Viridis", cmin=min(vfull), cmax=max(vfull)),
            showlegend=True, name=f"{girder_name} SFD", legendgroup=girder_name,
            text=[f"Node {nid}<br>X={x:.2f}<br>{force_key}={val:.2f}" for nid, x, val in zip(np.repeat(node_ids, 2)[1:-1], x_step, v_step)],
            hoverinfo="text"
        ))
...
def build_figure_bmd(ds, force_key, nodes, members, show_grid=True, scale=1.0, selected_girder="All"):
    ...
    for i, (gid, elems) in enumerate(sorted_girders):
        ...
        fig_bmd.add_trace(go.Scatter3d(
            ...
            legendgroup=girder_name + "_maxmin", showlegend=False, visible=False, hoverinfo="skip", meta="max_line"
        ))
        ...
        fig_bmd.add_trace(go.Scatter3d(
            ...
            legendgroup=girder_name + "_maxmin", showlegend=False, visible=False, hoverinfo="skip", meta="min_line"
        ))
    ...
    fig_bmd.update_layout(
        ...
        updatemenus=[
            dict(
                type="buttons", direction="right", x=0.5, y=1.15, showactive=False,
                buttons=[
                    dict(label="MAX", method="update", args=[{"visible": [True if getattr(t, 'meta', None) == "max_line" else t.visible for t in fig_bmd.data]}], args2=[{"visible": [False if getattr(t, 'meta', None) == "max_line" else t.visible for t in fig_bmd.data]}]),
                    dict(label="MIN", method="update", args=[{"visible": [True if getattr(t, 'meta', None) == "min_line" else t.visible for t in fig_bmd.data]}], args2=[{"visible": [False if getattr(t, 'meta', None) == "min_line" else t.visible for t in fig_bmd.data]}]),
