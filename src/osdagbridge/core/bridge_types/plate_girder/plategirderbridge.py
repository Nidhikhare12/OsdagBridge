...
class PlateGirderBridge:
    ...
    def _resolve_optimized_bounds_to_mm(self) -> None:
        ...
        def _resolve_one(full_key: str, initial_sizing_mm: float) -> None:
            ...
            # Enforce minimum flange thickness for fatigue compliance
            if 'flange_thickness' in full_key.lower():
                initial_sizing_mm = max(initial_sizing_mm, 28.0)

            if isinstance(raw, list) and raw:
                list_min = min(float(v) for v in raw)
                list_max = max(float(v) for v in raw)
                resolved_mm = max(list_min, min(list_max, initial_sizing_mm))
    ...
    def _convert_girder_dims_mm_to_m(self) -> None:
        ...
        def _to_m(full_key: str) -> None:
            ...
            if isinstance(val, dict):
                val = float(val.get('lower', 1000.0))
            elif isinstance(val, list) and val:
                val = min(float(v) for v in val)

            # Enforce minimum flange thickness for fatigue compliance
            if 'flange_thickness' in full_key.lower() and val != "All":
                try:
                    val = max(float(val), 28.0)
                except ValueError:
                    pass
    ...
    def _design_cross_bracing_members(self) -> dict:
        ...
        return design_cross_bracing(self)

    def _design_end_diaphragm_members(self) -> dict:
        from osdagbridge.core.bridge_types.plate_girder.end_diaphragm_design import design_end_diaphragm
        return design_end_diaphragm(self)
